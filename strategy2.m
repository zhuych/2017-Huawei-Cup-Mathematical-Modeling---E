function time = strategy2( position,strategy0 )
%UNTITLED5 此处显示有关此函数的摘要
%   此处显示详细说明
% strategy0 为两维向量，元素表示新加入的装载区域所在节点编号
...
time1=sum(Dist(1:24));
Z=[3 4 5 6 7 8 strategy0(1) strategy0(2)];
for j=1:8
for i=1:3
    Dist0(j,i)=graphshortestpath(GA,Z(j),position(i),'Method','Dijkstra');
    Dist1(j,i)=graphshortestpath(GA,Z(j),position(i+24),'Method','Dijkstra');
    Dist2(j,i)=Dist0(j,i)+Dist1(j,i);
end
for i=4:6
    Dist0(j,i)=graphshortestpath(GB,Z(j),position(i),'Method','Dijkstra');
    Dist1(j,i)=graphshortestpath(GB,Z(j),position(i+24),'Method','Dijkstra');
    Dist2(j,i)=Dist0(j,i)+Dist1(j,i);
end
for i=7:12
    Dist0(j,i)=graphshortestpath(GC,Z(j),position(i),'Method','Dijkstra');
    Dist1(j,i)=graphshortestpath(GC,Z(j),position(i+24),'Method','Dijkstra');
    Dist2(j,i)=Dist0(j,i)+Dist1(j,i);
end
for i=13:15
    Dist0(j,i)=graphshortestpath(GA,Z(j),position(i),'Method','Dijkstra');
...
Z7=find(Znum==7);   %去Z7装载的车编号
Z8=find(Znum==8);   %去Z8装载的车编号
timemax=max(time0)+1/6;
time2=24*timemax;
if (length(Z1)==1)
...
if (length(Z7)==1)
    time2=time2-timemax+time0(Z7(1)); 
elseif(length(Z7)>=2)
    dist0=sort(Dist0(7,Z7));
    dist1=sort(Dist1(7,Z7));
    time2=time2-2*timemax+dist0(1)+dist0(2)+dist1(1)+dist1(2);
    for i=3:length(Z7)
        time2=time2+max(dist0(i)+dist1(length(Z7)+3-i)-timemax,0);
    end
else
end
if (length(Z8)==1)
    time2=time2-timemax+time0(Z8(1)); 
elseif(length(Z8)>=2)
    dist0=sort(Dist0(8,Z8));
    dist1=sort(Dist1(8,Z8));
    time2=time2-2*timemax+dist0(1)+dist0(2)+dist1(1)+dist1(2);
    for i=3:length(Z8)
        time2=time2+max(dist0(i)+dist1(length(Z8)+3-i)-timemax,0);
    end
else
end
time=time1+time2;
