...
%将编号为qudiaod的节点去掉，即距离置为inf
qudiaod=[47,50,43,40,34,35,42,41,24,46,31,32,25,51,58,57,33,29,36,12,30,54,38,13,37,19,39,18,44,45,49,48,11,10,26,27,6,9,3,8,2,7,20,21,22,23]+8;
for i=1:length(qudiaod)
    qudiao=find(S==qudiaod(i));
    timeC(qudiao)=inf;
    qudiao=find(E==qudiaod(i));
    timeC(qudiao)=inf;
end
GA=sparse(S,E,timeA);
GB=sparse(S,E,timeB);
...
% H=view(P); %显示各个路段权值
for i=1:60     %第二波次发射节点Fi
    for j=1:60     %第一波次发射节点Fj
        if i==j
            Dist(i,j)=inf;
            Znum(i,j)=0;
        else
            for k=1:6   %转载节点Zk
                Dist0(k)=graphshortestpath(GC,k+2,i+8,'Method','Dijkstra')+graphshortestpath(GC,k+2,j+8,'Method','Dijkstra');
            end
            [Dist(i,j),Znum(i,j)]=min(Dist0);
        end
    end
end
[Dist1,inum]=min(Dist);
for j=1:length(inum)
    distan1=graphshortestpath(GC,1,j+8,'Method','Dijkstra');
    distan2=graphshortestpath(GC,2,j+8,'Method','Dijkstra');
    if distan1<distan2
        D0(j)=1;
        Dist2(j)=distan1+Dist1(j);
    else
        D0(j)=2;
        Dist2(j)=distan2+Dist1(j);
    end
end
[Dist3,jnum]=sort(Dist2);
J=jnum;    %时间对应的Fj
I=inum(jnum);    %时间对应的Fi
D=D0(jnum);  %时间对应的D1 or D2
for k=1:length(J)
    Z(k)=Znum(I(k),J(k));%时间对应的最优Zk
end
Dist3  
D
J
