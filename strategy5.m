function [time,longtime,repeat] = strategy5( position )
%UNTITLED5 此处显示有关此函数的摘要
%   repeat衡量经过节点的路径条数，表示节点重复使用率，要求尽量小
...
path0=zeros(3,24,20);
for i=1:3
    [Dist(i),path1]=graphshortestpath(GA,1,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(1,i,j)=path1(j);
    end
end
for i=4:6
    [Dist(i),path1]=graphshortestpath(GB,1,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(1,i,j)=path1(j);
    end
end
for i=7:12
    [Dist(i),path1]=graphshortestpath(GC,1,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(1,i,j)=path1(j);
    end
end
for i=13:15
    [Dist(i),path1]=graphshortestpath(GA,2,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(1,i,j)=path1(j);
    end
end
for i=16:18
    [Dist(i),path1]=graphshortestpath(GB,2,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(1,i,j)=path1(j);
    end
end
for i=19:24
    [Dist(i),path1]=graphshortestpath(GC,2,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(1,i,j)=path1(j);
    end
end
...
for i=1:3
    [Distt,path1]=graphshortestpath(GA,Znum(i)+2,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(2,i,j)=path1(j);
    end
    [Distt,path1]=graphshortestpath(GA,Znum(i)+2,position(i+24),'Method','Dijkstra');
    for j=1:length(path1)
        path0(3,i,j)=path1(j);
    end
end
for i=4:6
    [Distt,path1]=graphshortestpath(GB,Znum(i)+2,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(2,i,j)=path1(j);
    end
    [Distt,path1]=graphshortestpath(GB,Znum(i)+2,position(i+24),'Method','Dijkstra');
    for j=1:length(path1)
        path0(3,i,j)=path1(j);
    end
end
for i=7:12
    [Distt,path1]=graphshortestpath(GC,Znum(i)+2,position(i),'Method','Dijkstra');
    for j=1:length(path1)
        path0(2,i,j)=path1(j);
    end
    [Distt,path1]=graphshortestpath(GC,Znum(i)+2,position(i+24),'Method','Dijkstra');
    for j=1:length(path1)
        path0(3,i,j)=path1(j);
    end
end
...
repeat=0;
for i=1:62      %考虑道路节点重叠路径情况
    num(i)=length(find(path0==i+68));
    repeat=repeat+num(i)^2;
end
repeat=repeat/sum(num);   %归一化，使长路径与短路径分散程度相同
...
timemax=max(time0)+1/6;   %假设通过合理优化，第二波次耗时最长的车装载时无须排队
longtime=max(Dist+time0); %假设通过合理优化，两波次耗时最长的车装载时无须排队
time2=24*timemax;
if (length(Z1)==1)
